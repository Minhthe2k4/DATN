...
// Repository quản lý dữ liệu bài học (Lesson).
// Hỗ trợ các truy vấn thống kê độ khó, trạng thái Premium và quản lý dữ liệu rác.
public interface LessonRepository extends JpaRepository<Lesson, Long> {
    // Xóa vĩnh viễn bài học khỏi cơ sở dữ liệu.
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM lessons WHERE id = :id", nativeQuery = true)
    void hardDelete(Long id);

    // Khôi phục bài học đã bị xóa mềm bằng cách reset trường deleted_at.
    @Modifying
    @Transactional
    ...
    // Tính toán độ khó trung bình trên toàn bộ kho bài học phục vụ báo cáo.
    ...
    // Lấy danh sách bài học kèm theo thông tin gán chủ đề phục vụ trang quản lý.
    @Query("""
                    select l.id as id,
                           l.name as name,
    ...
    // Truy vấn các bài học đã bị xóa mềm phục vụ chức năng thùng rác.
    @Query(value = """
                    select l.id as id,
                           l.name as name,
                           l.description as description,
