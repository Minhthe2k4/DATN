...
public interface UserVocabularyLearningRepository extends JpaRepository<UserVocabularyLearning, Long> {
    ...
    @Query("""
            ...
            """)
    List<UserVocabularyLearning> findDueReviewsByUserId(@Param("userId") Long userId, @Param("now") java.util.Date now);
    ...
    long countByNextReviewGreaterThanEqualAndNextReviewLessThan(@Param("startDate") java.util.Date startDate, @Param("endDate") java.util.Date endDate);
