...
import com.example.DATN.repository.user.UserFavoriteRepository;
import com.example.DATN.repository.user.UserProgressRepository;
...
public class UserInteractionController {
    ...
    public ResponseEntity<?> toggleFavorite(
        ...
        Optional<UserFavorite> existing = userFavoriteRepository.findByUserIdAndTargetIdAndTargetType(userId, targetId,
                targetType);
    ...
    public ResponseEntity<?> updateProgress(
        ...
        UserProgress progress = userProgressRepository
                .findByUserIdAndTargetIdAndTargetType(userId, targetId, targetType.toUpperCase())
                .orElse(new UserProgress(userRepository.findById(userId).orElseThrow(), targetId,
                        targetType.toUpperCase(), 0.0));

        // Chỉ cập nhật nếu tiến độ mới cao hơn tiến độ cũ (hoặc cho phép cập nhật lùi
        // tùy ý? Thường là chỉ tiến tới)
    ...
    public ResponseEntity<?> getStats(
        ...
        stats.put("isFavorite", userFavoriteRepository.existsByUserIdAndTargetIdAndTargetType(userId, targetId,
                targetType.toUpperCase()));

        Optional<UserProgress> progress = userProgressRepository.findByUserIdAndTargetIdAndTargetType(userId, targetId,
                targetType.toUpperCase());
    ...
    public ResponseEntity<?> getBulkStats(
        ...
        for (Long id : targetIds) {
            Map<String, Object> stats = new HashMap<>();
            stats.put("isFavorite", userFavoriteRepository.existsByUserIdAndTargetIdAndTargetType(userId, id,
                    targetType.toUpperCase()));

            Optional<UserProgress> progress = userProgressRepository.findByUserIdAndTargetIdAndTargetType(userId, id,
                    targetType.toUpperCase());
            stats.put("progressPercent", progress.map(p -> p.progressPercent).orElse(0.0));
